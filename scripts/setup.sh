...
if [ "$DO_FRONTEND" -eq 1 ]; then
    echo "==> frontend-truthbets npm install (project-local cache $ROOT/.npm-cache)"
    (cd "$ROOT/frontend-truthbets" && npm install --no-fund --no-audit --cache "$ROOT/.npm-cache")
fi

if [ "$DO_WARMUP" -eq 1 ]; then
    echo "==> genvm-lint first-run warmup"
    "$VENV/bin/genvm-lint" check contracts/truth_bets.py >/dev/null 2>&1 || true
fi
